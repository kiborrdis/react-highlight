import { useState, useRef, useMemo, type CSSProperties, useEffect, useCallback } from "react";

export const useInputHightlight = ({ textColor }: { textColor: string; }) => {
  ...
  const preEl = useRef<HTMLDivElement | null>(null);
  ...
  const syncScroll = useCallback(() => {
    if (el && preEl.current) {
      preEl.current.scrollLeft = el.scrollLeft;
    }
  }, [el]);
  ...
  return {
    ...
    setTextarea: useCallback((el: HTMLInputElement | null) => {
      setEl(el);
